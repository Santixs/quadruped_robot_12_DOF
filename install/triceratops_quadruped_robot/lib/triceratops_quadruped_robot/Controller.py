...
class RobotControl:
    def __init__(self):
        ...
        # Add subscribers for computer vision events
        self.wave_sub = rclpy.create_node('wave_detector_subscriber')
        self.wave_sub.create_subscription(
            Bool, 
            'wave_detected', 
            self.wave_detected_callback, 
            10
        )
        
        self.box_sub = rclpy.create_node('box_detector_subscriber')
        self.box_sub.create_subscription(
            Bool, 
            'box_detected', 
        ...
        self.executor.add_node(self.wave_sub)
        self.executor.add_node(self.box_sub)
    ...
    def wave_detected_callback(self, msg):
        """Handle wave detection events"""
        if msg.data:
            # Switch to puppy mode when wave is detected
            print("Wave detected! Switching to puppy_move mode")
            self.switch_mode_service.mode = 'puppy_move'
    
    def box_detected_callback(self, msg):
        """Handle box detection events"""
        if msg.data:
            # Note: The mouth movement is directly handled by the vision node
            # by publishing to the body_pose topic
            print("Box detected! Opening mouth")
